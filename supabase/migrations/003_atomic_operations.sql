...
-- 1. consume_points の更新版（auth.uid() 検証を追加）
...
-- 2. save_learning_event_atomic — 学習イベント保存を1トランザクションに
...
BEGIN
  ...
  VALUES (p_user_id, 'consume_memo', -p_points_to_consume, v_balance - p_points_to_consume, jsonb_build_object('learning_event_id', v_event_id));

  RETURN jsonb_build_object('success', true, 'learning_event_id', v_event_id);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 3. submit_review_atomic — 復習提出を1トランザクションに
...
BEGIN
  ...
  INSERT INTO point_transactions (user_id, type, amount, balance_after, metadata)
  VALUES (p_user_id, 'consume_text_answer', -p_points_to_consume, v_balance - p_points_to_consume, jsonb_build_object('srs_card_id', p_card_id));
